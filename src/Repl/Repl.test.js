...
test ('parse an ID expression', ()=> {
    const p = parseProgram('x');
    expect(p.type === ExprType.Id);
    expect(p.name === 'x');
})

test('parse a let expression', ()=> {
    const p = parseProgram('(let ((x 5)) x)');
    expect(p.type === ExprType.Let);

    const pairs = p.bindingPairs;
    expect(pairs.length === 1);
    expect(pairs[0][0] === 'x');
    expect(pairs[0][1].type === ExprType.Num);
    expect(pairs[0][1].value === 5);

    expect(p.body.type === ExprType.Id);
    expect(p.body.name === 'x');
...
test ('interpert a let with a more complex named sub expression', ()=> {
    const p = parseProgram('(let ((x (* 5 5))) (+ x 5))');
    expect(interp(p, {}) === 30);
})